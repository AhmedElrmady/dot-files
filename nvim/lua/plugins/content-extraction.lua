...
return {
  ...
  config = function()
            ...
            vim.fn.jobstart(title_cmd, {
              ...
              on_stdout = function(_, title_data)
                if title_data and #title_data > 0 then
                  ...
                  local enhance_cmd = string.format(
                    'cd "%s" && echo %s | python3 ai_content_enhancer.py --title "%s" --content-from-stdin --url "%s"',
                    config.scripts_path,
    ...
    vim.api.nvim_create_user_command(
      "ExtractURLEnhanced",
      extract_url_enhanced,
      { desc = "Extract URL with AI enhancement" }
    )
